...
namespace EAVServiceTest
{
    public partial class EAVServiceTestHarness
    {
        ...
        public void CreateChildContainer()
        {
            ...
            HttpResponseMessage response = WebClient.PostAsJsonAsync<EAV.Model.BaseEAVContainer>(String.Format("api/meta/containers/{0}/containers", dbParentContainer.Container_ID), new EAV.Model.BaseEAVContainer() { Name = childContainerName, DataName = childContainerName.ToUpper(), DisplayText = childContainerName + ":", IsRepeating = true }).Result;
            if (response.IsSuccessStatusCode)
            {
                var container = response.Content.ReadAsAsync<EAV.Model.BaseEAVContainer>().Result;
        ...
        public void RetrieveAttributes()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                var attributes = response.Content.ReadAsAsync<IEnumerable<EAV.Model.BaseEAVAttribute>>().Result;
        ...
        public void CreateAttribute()
        {
            ...
            HttpResponseMessage response = WebClient.PostAsJsonAsync<EAV.Model.BaseEAVAttribute>(String.Format("api/meta/containers/{0}/attributes", dbContainer.Container_ID), new EAV.Model.BaseEAVAttribute() { Name = attributeName, DataName = attributeName.ToUpper(), DisplayText = attributeName + ":", IsKey = true }).Result;
            if (response.IsSuccessStatusCode)
            {
