...
namespace EAVServiceTest
{
    public partial class EAVWebServiceTestHarness
    {
        ...
        public void UpdateSubject()
        {
            ...
            HttpResponseMessage response = WebClient.PatchAsJsonAsync<EAVStoreLibrary.StoreSubject>("api/data/subjects", subject).Result;
            if (response.IsSuccessStatusCode)
            {
        ...
        public void CreateRootInstance()
        {
            ...
            HttpResponseMessage response = WebClient.PostAsJsonAsync<EAVStoreLibrary.StoreInstance>(String.Format("api/data/subjects/{0}/instances?container={1}", dbSubject.Subject_ID, dbContainer.Container_ID), new EAVStoreLibrary.StoreInstance()).Result;
            if (response.IsSuccessStatusCode)
            {
